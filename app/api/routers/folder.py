...
def update_folder(user_id: int, folder_id: int, folder: FolderUpdate, context: AppContextAuth = Depends(get_auth_context)):
    return FolderApi(context).update(user_id, folder_id, folder)
...
def delete_folder(user_id: int, folder_id: int, force: bool, context: AppContextAuth = Depends(get_auth_context)):
    return FolderApi(context).delete(user_id, folder_id, force)


@router.get("", response_model=List[FolderOut], description="Get all folders for user")
def get_all(user_id: Optional[int] = Query(None), context: AppContextAuth = Depends(get_auth_context)):
...
def get(user_id: int, id: int, context: AppContextAuth = Depends(get_auth_context)):
    return FolderApi(context).get(user_id, id)
